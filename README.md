...
- 交接班盘点会话冻结开始时的批次余额与在途卸货快照，接收逐罐测量并按油品汇总差异；
- 容差内差异由当班调度结清，超容差必须由另一名风险人员选择调整、拆分调查或拒绝，盘点期间批次仍可发运，结论按会话版本和余额令牌检测冲突；
- 盘点调整幂等落账，保留逐批次前后余额、原因和哈希签署链，会话状态区分未测量、测量中、待复核、已调整、拆分调查和已驳回；
...
健康检查为 `GET /health`。除健康检查外，请求通过 `X-Actor-Id` 携带操作者编号。可用接口覆盖报价、设施、线路、停运事件、库存批次、提名、能力分配、发运、供应情景、交接班盘点（`/stock-counts` 及其 `measurements`、`settle`、`review`、`adjustment-chain` 子路径）和审计链。服务重启后，SQLite 中的业务状态和历史版本会继续保留。
